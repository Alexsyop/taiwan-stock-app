...
台股分析系統 v6.1
=================
Gemini 職責：只負責搜尋下市風險股票，不調整任何評分
評分機制：完全使用原本量化邏輯（技術+籌碼+基本面）
硬風控：全額交割/下市警告 → 強制 C 評等
"""
...
NAMES = {
    ...
    "3533":"嘉澤","4958":"臻鼎-KY","5269":"祥碩","6271":"同欣電","4919":"新唐","6526":"達發",
}
...
# ─────────────────────────────────────────────────────────────
# ★ Gemini：只負責搜尋下市風險股票（不調整任何評分）
# ─────────────────────────────────────────────────────────────
def gemini_fetch_delisting(api_key):
    """
    使用 Gemini 搜尋台股下市風險/全額交割股票清單。
    只回傳股票代號集合，完全不涉及評分。
    結果存入 session_state，6小時內不重複呼叫。
    """
    if not api_key:
        return set()
    ...
    try:
        ...
        prompt = """你是台灣股市專家。請根據你的知識，提供目前（最近期）台灣股市中：
1. 全額交割股票（須全額繳款，流動性極差）
2. 已收到下市警告或面臨強制下市的股票

請只列出4位數字的台股代號（上市+上櫃都列），每行一個，不需要股票名稱或說明。
如果不確定，寧可不列也不要列錯。
格式範例：
1234
5678"""
...
def fetch_full_delivery_cached():
    ...
    out = set()
    try:
        r = requests.get("https://openapi.twse.com.tw/v1/announcement/fullCash",
...
def fetch_delisting_cached():
    ...
    out = set()
    try:
        r = requests.get("https://openapi.twse.com.tw/v1/announcement/changeTrading",
                         headers=HDR, timeout=10, verify=False)
        if r.status_code == 200:
            for item in r.json():
...
def fetch_twse_institution_all():
    """
    TWSE T86 三大法人（修正版）
    自動嘗試最近5個交易日，直到拿到有效數據為止。
    """
    out = {}
    trading_days = last_trading_days(5)
    ...
    for trade_date in trading_days:
        ...
        for url in [
            ...
            try:
                ...
                if not rows or len(rows) < 5: continue  # 有意義的數據才接受

                for row in rows:
                    if len(row) < 11: continue
                    sid = str(row[0]).strip()
                    if not sid.isdigit(): continue
                    f_net = parse_tw(row[7])  // 1000  # 外陸資(不含自營)
                    t_net = parse_tw(row[10]) // 1000  # 投信
                    d_net = parse_tw(row[14]) // 1000 if len(row)>14 else 0  # 自營
                    out[sid] = {"f":f_net,"t":t_net,"d":d_net,
                                "name":str(row[1]).strip() if len(row)>1 else ""}

                if len(out) > 100:  # 有足夠的數據
                    return out
            except Exception:
                continue
        if len(out) > 100:
...
def fetch_tpex_institution_all():
    ...
    for trade_date in trading_days:
        date_str = trade_date.strftime("%Y/%m/%d")
        try:
            url = f"https://www.tpex.org.tw/web/fund/tripleFund/index.php?l=zh-tw&o=json&d={date_str}"
            r   = requests.get(url, headers=HDR, timeout=20, verify=False)
            if r.status_code != 200: continue
            d   = r.json()
...
def get_rev(sid,token):
    ...
    recs=[{"rev":ff(r.get("revenue",0)),"yr":fi(r.get("revenue_year",0)),
...
def analyze(sid, token, disposed, full_delivery, delisting, gemini_del, force=False):
    ...
    # 硬風控判定（全額交割/下市警告/Gemini抓到的風險股）
    hard_risk_set = delisting | full_delivery | gemini_del
    ...
    try:
        ...
        # ── 硬風控覆蓋（全額交割/下市 → 強制C，不調量化分數）
        if is_hard_risk:
            rt="C"; lb="避　開"
            if is_full_del:
                neg.insert(0,"🚨 全額交割股：流動性極差，強制C評等，禁止操作")
            if is_delisting:
                neg.insert(0,"⚠️ 下市警告：面臨下市風險，強制C評等")
        ...
        result={
            ...
            "is_delisting":is_delisting,"is_hard_risk":is_hard_risk,
            "date":last["date"],
        }
...
def run_scanner():
    ...
    gemini_del=st.session_state.gemini_delisting
    ...
    return (hot[:20],trust[:15],launch[:10],
            date.today().strftime("%Y/%m/%d"),
            len(prices),len(insts))
...
def chip_html(label,net,con_d):
    ...
    ct=(f"連買{con_d}日" if con_d>=2 else "今日買超" if con_d==1
        else "今日中性" if con_d==0 else "今日賣超" if con_d==-1 else f"連賣{abs(con_d)}日")
...
def build_wiwynn(r):
    ...
    if ry and ry>20: tags+=f'<span class="tag tag-g">YoY+{ry:.0f}%</span>'

    # 硬風控橫幅
    hard_banner=""
    if r.get("is_delisting"):
        hard_banner='<div class="hard-risk-banner"><strong>⚠️ 下市警告：強制 C 評等</strong><span>此股票面臨下市風險（TWSE官方或 Gemini 偵測），禁止操作，靜待後續公告。</span></div>'
    elif r.get("is_full_del"):
        hard_banner='<div class="hard-risk-banner"><strong>🚨 全額交割股：強制 C 評等</strong><span>全額交割股每筆交易須全額繳款，流動性幾乎為零，嚴禁操作。</span></div>'
...
<div class="tp-lbl"><span>0</span><span>現價{p:,.0f}</span><span>{tp:,.0f}</span></div>{upbox}</div>"""
    ...
    evs=[]
    if r.get("is_delisting"):    evs.append(("#e74c3c","⚠️ 下市警告","TWSE官方/Gemini偵測：面臨下市風險，禁止操作。"))
...
<div class="footer">{r['name']}（{sid}）| {r.get('date',today)} | 評分{sc}/100（純量化）| {ts_}<br>
數據：FinMind API | 下市偵測：TWSE OpenAPI + Gemini | 僅供參考，不構成投資建議</div>
</div></body></html>"""
...
<div style="font-size:10px;color:#8fa3b8;margin-bottom:4px">📊 台股分析 {today} | S:{cnts['S']} A:{cnts['A']} B:{cnts['B']} C:{cnts['C']}</div>{nav}
...
def tab_scanner():
    ...
    c1.info("🆓 免費，無需 Token")
    c2.info("⏱ 快取4小時，自動找最近交易日")
    c3.info("🤖 Gemini偵測下市股（選填）")
    ...
    if st.button("🔄 重新整理全市場數據",use_container_width=True):
        ...
        fetch_full_delivery_cached.clear(); fetch_delisting_cached.clear()
    ...
    st.caption(f"資料：{inst_date} | 上市+上櫃股價：{n_p} 檔 | 法人數據：{n_i} 檔")

    if n_i == 0:
        st.error("⚠️ 法人數據目前無法取得（可能是假日或 TWSE 尚未更新）。籌碼掃描需要法人買賣數據才能運作。")
        st.info("請在台灣股市開收盤後（下午3點後）重試，或點上方「重新整理」按鈕。")
        return
    ...
    if hot:
        df=pd.DataFrame([{"熱度":r["tier"],"代號":r["sid"],
                          "名稱":r["name"][:8],"⚠":r["tag"],
        ...
        st.info("今日無符合條件個股（外資+投信同向，吸籌率≥5%）")
    ...
    if launch:
        df=pd.DataFrame([{"代號":r["sid"],"名稱":r["name"][:8],"⚠":r["tag"],
                          "收盤":r["price"],"漲跌%":f"{r['chg']:+.1f}%",
                          "今日投信":f"+{r['t']:,}張","金額":f"{r['val_yi']:.2f}億",
    ...
    st.caption("5日內投信買超天數≥4，來自「個股分析」的分析結果")
    if trust:
        ...
        st.info("請先在「🔍 個股分析」執行分析，結果將自動出現在此")
...
def tab_analysis():
    ...
    if run:
        ...
        with st.spinner("取得風控清單..."):
            ...
            if gkey:
                with st.spinner("🤖 Gemini 偵測下市股..."):
                    gemini_del=gemini_fetch_delisting(gkey)
                    if gemini_del:
                        st.info(f"🤖 Gemini 偵測到 {len(gemini_del)} 支下市風險股")
            else:
                gemini_del=set()
...
def tab_settings():
    ...
    with st.form("sf",clear_on_submit=False):
        ...
        st.markdown("""**🤖 Gemini API Key**（選填）
用途：搜尋下市風險/全額交割股票清單，**不調整任何評分**""")
    ...
    if st.session_state.gemini_delisting:
        st.warning(f"🤖 Gemini 目前偵測到的下市風險股：{', '.join(sorted(st.session_state.gemini_delisting))}")
...
```
    ...
    if st.button("🗑 清除所有快取",use_container_width=True):
        ...
        for fn in [fetch_twse_prices_all,fetch_twse_institution_all,
            ...
            fn.clear()
        st.session_state.results=[]; st.session_state.gemini_delisting=set(); st.success("✅ 已清除")
