...
NAMES={
    ...
    "3034":"聯詠","3037":"欣興","3231":"緯創","2313":"華通",
    "1513":"中興電","3293":"鈊象","3081":"聯惠","2059":"川湖",
    # ── 上櫃（TPEx）─────────────────────────
    "6515":"穎威","5274":"信驊","3017":"奇鋐","3048":"益登",
    "3529":"力旺","3665":"貿聯-KY","3167":"達亮","6510":"精測",
    "3443":"創意","3035":"智原","3034":"聯詠","3406":"玉晶光",
    "3008":"大立光","4966":"譜瑞-KY","6116":"彩晶","6239":"力成",
    "6488":"環球晶","6533":"晶心科","6770":"力積電",
    "5285":"界霖","4743":"合一","4968":"立積",
    "6230":"超豐","8299":"群聯","5347":"世界先進",
    "3533":"嘉澤","4958":"臻鼎-KY","5269":"祥碩",
    "6271":"同欣電","4919":"新唐","3550":"樂士",
    "4743":"合一","5274":"信驊","6526":"達發",
}
...
# ================================================================
# ★ Fix-4：Yahoo Finance 分析師共識目標價
# ================================================================
def get_yahoo_analyst_target(stock_id: str) -> dict | None:
    """
    用 yfinance 抓取 Yahoo Finance 分析師共識目標價。
    自動嘗試 .TW（上市）/ .TWO（上櫃）兩種後綴。
    回傳 {target, high, low, count, logic, source} 或 None。
    """
    try:
        import yfinance as yf
        import pandas as pd
    ...
    for suffix in [".TW", ".TWO"]:
        try:
            ...
            # 驗證：regularMarketPrice 存在才代表抓到真實數據
            ...
            if mean_price and not pd.isna(mean_price) and float(mean_price) > 0:
                n = int(count) if count else 0
                return {
                    "target": round(float(mean_price), 2),
                    "high":   round(float(high_price), 2) if high_price and not pd.isna(high_price) else None,
                    "low":    round(float(low_price),  2) if low_price  and not pd.isna(low_price)  else None,
                    "count":  n,
...
def get_disposed():
    ...
    try:
        ...
        if r2.status_code == 200:
            for i in r2.json():
                # TPEx 欄位名稱可能是 SecuritiesCode 或 code
  ...
  <div style="margin-top:10px">
    ...
    if tp and p:
        ...
        if tp_count and tp_count > 0:
            source_badge=(f'<span style="background:#edf4ff;color:#2980b9;border:1px solid #aec6ea;'
                          f'font-size:10px;padding:1px 7px;border-radius:99px;margin-left:6px;font-weight:600">'
                          f'👥 {tp_count} 位分析師</span>')

        # 高低區間條（若有 Yahoo 分析師數據）
        range_bar=""
        if tp_high and tp_low and tp_high > tp_low:
            rng=tp_high-tp_low
            curr_pct=min(max((p-tp_low)/rng*100,0),100)
            mean_pct=min(max((tp-tp_low)/rng*100,0),100)
            range_bar=f"""
<div style="margin-top:10px">
  <p style="font-size:10px;font-weight:600;color:#666;margin-bottom:6px;text-transform:uppercase">
    分析師目標價區間（低/均/高）
  </p>
  <div style="position:relative;background:#f0f0f0;border-radius:6px;height:14px;margin-bottom:4px">
    <!-- 區間底色 -->
    <div style="position:absolute;left:{min(max((tp_low-tp_low)/(tp_high-tp_low)*100,0),100):.0f}%;
      width:100%;height:14px;background:linear-gradient(90deg,#fdf2f2,#f0f9f4,#fdf2f2);
      border-radius:6px"></div>
    <!-- 平均目標線 -->
    <div style="position:absolute;left:{mean_pct:.0f}%;top:-4px;width:3px;height:22px;
      background:#2980b9;border-radius:2px;z-index:2"></div>
    <!-- 現價針 -->
    <div style="position:absolute;left:{curr_pct:.0f}%;top:-4px;width:3px;height:22px;
      background:#1a2332;border-radius:2px;z-index:3"></div>
  </div>
  <div style="display:flex;justify-content:space-between;font-size:10px;color:#888">
    <span>低 {tp_low:,.0f}</span>
    <span style="color:#2980b9;font-weight:600">均 {tp:,.0f}</span>
    <span>高 {tp_high:,.0f}</span>
  </div>
  <div style="font-size:10px;color:#555;margin-top:4px;text-align:center">
    ■ <span style="color:#1a2332">現價</span>　
    ■ <span style="color:#2980b9">分析師均值</span>
  </div>
</div>"""

        tp_card=f"""
<div class="card">
  <p class="card-title">💰 目標價分析{source_badge}</p>
  <div class="row"><span class="row-l">資料來源</span>
    <span class="row-v" style="font-size:12px;color:#555">{ts_}</span></div>
  <div class="row"><span class="row-l">分析師均值目標</span>
    <span class="row-v {'dn' if exceed else 'up'}" style="font-size:16px;font-weight:700">
    {tp:,.0f} 元</span></div>
  {"<div class='row'><span class='row-l'>最高目標</span><span class='row-v'>"+f"{tp_high:,.0f} 元</span></div>" if tp_high else ""}
  {"<div class='row'><span class='row-l'>最低目標</span><span class='row-v'>"+f"{tp_low:,.0f} 元</span></div>" if tp_low else ""}
  <div class="row"><span class="row-l">現價 vs 均值目標</span>
    <span class="row-v {'dn' if exceed else 'up'}"><strong>{up2:+.1f}%</strong></span></div>
  {"<div class='row'><span class='row-l'>分析師人數</span><span class='row-v'>"+str(tp_count)+"位</span></div>" if tp_count else ""}
  {range_bar}
  <div class="tp-bar-wrap" style="margin-top:10px">
